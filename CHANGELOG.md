...
- **FastAPI recipes 02, 04, and 05 were missing `email-validator` (#122)**: their schemas use pydantic's `EmailStr`, which requires `email-validator`, but it was not in their `requirements.txt`, so the apps and their tests failed on import after a clean install. Added it.
